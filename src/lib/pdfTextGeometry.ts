...
import type { TextItem as PdfTextItem } from "../types/pdfItems";
...
export function pdfTextItemToPdfBounds(
  item: PdfTextItem,
...
export function pdfBoundsToViewportRect(
  ...
  const pts = [
    viewport.convertToViewportPoint(bounds.minX, bounds.minY) as [
      number,
      number,
    ],
    viewport.convertToViewportPoint(bounds.maxX, bounds.minY) as [
      number,
      number,
    ],
    viewport.convertToViewportPoint(bounds.maxX, bounds.maxY) as [
      number,
      number,
    ],
    viewport.convertToViewportPoint(bounds.minX, bounds.maxY) as [
      number,
      number,
    ],
  ];
