...
export function refreshFabricFormField(
  ...
  const target = canvas
    .getObjects()
    .find((o) => isFormFieldObject(o) && getFormFieldId(o) === meta.id) as
    | Group
    | undefined;
  if (!target) return;
  ...
  const { width, height, left, top } = formFieldMetaToPixels(
    meta,
    canvasW,
    canvasH,
  );
  ...
  const label = objects[1] as FabricText;

  if (!rect || !label) return;

  // 1. Update the rect (item 0)
  const r = Math.min(width, height) / 2;
  rect.set({
    left: 0,
    top: 0,
