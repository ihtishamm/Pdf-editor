...
function syncFormPlacementGhost(
  ...
  const existing = canvas.getObjects().find(isFormGhostObject) as
    | Group
    | undefined;
  if (!existing) {
    const rect = new Rect({
...
export function attachFabricCanvasTools(
  ...
  function syncSelection() {
    ...
    const d = (
      obj as FabricObject & { data?: { tool?: string; fieldId?: string } }
    )?.data;
  ...
  const onMouseDown = (opt: TPointerEventInfo) => {
    ...
    // Use canvas.getScenePoint which is more reliable for coordinate mapping in some setups
...
export function isOverlayPropertiesObject(
  o: FabricObject | undefined,
): boolean {
  return isShapeToolObject(o) || isAnnotateToolObject(o) || isWhiteoutObject(o);
}
