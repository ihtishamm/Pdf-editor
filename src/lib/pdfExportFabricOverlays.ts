...
import {
  padPdfBounds,
  type PdfNativeTextOriginalBounds,
...
function parseColor(fill: unknown): { r: number; g: number; b: number } | null {
...
export async function drawFabricOverlaysOnPdfPages(
  ...
  for (let pageIndex = 0; pageIndex < pageCount; pageIndex++) {
    ...
    // Draw masked regions from store
    const masks = maskedRegions?.get(pageNum) ?? [];
    for (const region of masks) {
      const b = padPdfBounds(region.bounds);
      const fill = parseColor(region.fill) || { r: 1, g: 1, b: 1 };
